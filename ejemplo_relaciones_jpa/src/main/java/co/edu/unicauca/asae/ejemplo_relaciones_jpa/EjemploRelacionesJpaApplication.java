...
public class EjemploRelacionesJpaApplication implements CommandLineRunner {
	...
	public void buscarUsuario() {
		Optional<Usuario> optional = this.servicioBDUsuarios.findById(1);
		if (optional.isPresent()) {
			Usuario objUsuario = optional.get();
			System.out.println("Usuario: " + objUsuario.getNombre());
			System.out.println("Perfiles asignados");
			for (Perfil objPerfil : objUsuario.getListaPerfiles()) {
				System.out.println(objPerfil.getPerfil());
			}
		} else {
	...
	public void crearusuarioConDosPerfiles() {
		Usuario objUsuario = new Usuario();
		objUsuario.setNombre("Sandra Lopez");
		objUsuario.setEmail("danielp@unicauca.edu.co");
		objUsuario.setFechaRegistro(new Date());
